...
| `.github/workflows/production-readonly-audit.yml` | `production-readonly-audit.sh` | `DEPLOY_DEPENDENCY` | Current production-readiness workflow; script now validates the fixed runtime-bound recovery selector and independent backup evidence. Production readiness remains fail-closed until authorized live evidence validates. |
...
| `scripts/production-readonly-audit.sh` | `current-successful-deployment.v1`, recovery receipt and bundle, `production-readonly-audit.yml` | `DEPLOY_DEPENDENCY` | Validates one exact active-runtime selector and recovery bundle; missing/ambiguous evidence is NOT_EVALUATED and independent backup readiness remains fail-closed |
...
| `scripts/tests/production-readonly-audit.test.sh` | `production-readonly-audit.sh`, independent object-copy receipt, backup readiness | `TEST_DEPENDENCY` | Read-only audit regression coverage; selector and recovery-bundle edge cases are additionally covered by `production-readonly-audit-selector.test.sh` |
