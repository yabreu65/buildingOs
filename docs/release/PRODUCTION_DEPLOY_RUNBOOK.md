...
Rollback does not check out old source or rebuild old images. It stops the current API before checking migration count or data compatibility and keeps it quiescent until it recreates only API/Web from the captured digests; it then verifies the unchanged migration count and runs health smoke.
