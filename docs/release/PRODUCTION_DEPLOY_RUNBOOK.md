...
| Seeds | Prohibited |
| Evidence | External 0600 SUCCESS record, atomic current-runtime selector, recovery-point reference, SHA, image digests, backup and migration count |
...
The workflow sends the versioned deploy script, security validator, storage guard, exact `infra/production/backup-postgres.identity.v1` manifest, and the explicit non-symlink-validated recovery-helper dependency closure under `scripts/lib/` as a temporary remote control bundle outside the checkout. It does not transfer the repository, environment files, or secrets. The bundle is removed when the remote command exits. Deployment stops unless the external backup script is a non-symlink regular file at the exact path, SHA-256, owner, group, and mode declared by the canonical manifest. Immediately before backup, the validator creates a private copy, verifies its SHA-256, revalidates the source identity, and executes only the pinned copy.
...
Before migrations, the deploy control preflights the recovery-point capture, quiesces the API, applies and behaviorally probes a temporary S3 write-deny fence (including protection against outstanding presigned PUT URLs), captures the shared PostgreSQL/File snapshot and exact referenced object bytes, validates the receipt and bundle, restores the exact prior bucket policy, and resumes only an API that was previously running. Any uncertainty blocks later backup/checkpoint/checkout/migration steps. The current-runtime selector at `/opt/pawtech/apps/buildingos/deployments/current-successful-deployment.v1` is atomically published only after the SUCCESS deployment record and exact active checkout/API/Web identities are verified. A failed deploy preserves the prior selector. The read-only audit follows that single selector; it does not choose by recency or directory name.
...
Rollback does not check out old source or rebuild old images. It stops the current API before checking migration count or data compatibility and keeps it quiescent until it atomically records an `IN_PROGRESS` rollback and recreates only API/Web from the captured digests; if validation stops before recreation, the current API is restored. A later retry accepts an interrupted rollback only when both running image digests match either the recorded predecessor or source state; a matching quiesced source state is resumed in recovery mode, and any other mismatch stops closed. It then verifies the unchanged migration count, active API/Web digests and revisions, and health smoke before publishing the selector atomically. Recovery evidence is copied only from exactly one successful prior record matching the rollback target runtime identity; no/multiple matches leave recovery status `NOT_EVALUATED` without guessing.

Immediately before rollback, the script reuses the same read-only compatibility guard used by deploy. For the historical exact 97->98 receipt-snapshot delta, compatibility requires zero rows in the six new `Payment` receipt snapshot and generation-state fields; existing pre-98 Finance data does not block this narrow path. For the current 97->99 schema and migration delta, the fallback predicate requires zero rows using target-only Finance structures: cross-currency snapshots/rates, shared recurring expenses, funds, income applications/policies, valued liquidations and income offsets. If the applicable predicate finds data, the previous application is no longer considered compatible and rollback stops without changing runtime or database.
