...
describe('QueryExecutorService', () => {
  ...
  describe('execute', () => {
    it('logs execution time and query shape via AssistantFeedbackService', async () => {
      ...
      await service.execute(plan, 'tenant-1', ['OPERATOR']);
    ...
    it('returns raw data from executor', async () => {
      ...
      const result = await service.execute(plan, 'tenant-1', ['OPERATOR']);
  ...
  describe('RBAC enforcement', () => {
    it('throws when user lacks required permission', async () => {
      ...
      await expect(service.execute(plan, 'tenant-1', ['RESIDENT'])).rejects.toThrow();
    });
  });
  ...
  describe('error handling', () => {
    it('handles Prisma errors gracefully', async () => {
      ...
      await expect(service.execute(plan, 'tenant-1', ['OPERATOR'])).rejects.toThrow('Prisma error');
    });
  });
