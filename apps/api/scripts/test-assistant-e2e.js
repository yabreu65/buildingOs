/**
 * Script de Test E2E Simple - Asistente AI San Cristóbal
 * Usa fetch directo contra la API (requiere servidor levantado)
 */
...
async function login() {
  ...
  if (!membership) {
    throw new Error('Usuario no tiene membership');
  }

  console.log(`✅ Logueado como ${data.user.email}`);
...
async function ask(token, tenantId, category, id, question, expectedKeywords = []) {
  ...
  try {
    ...
    return {
      ...
      question,
      status,
      response: data.answer?.substring(0, 180) || '',
      latencyMs,
      error,
    };
  } catch (err) {
...
async function runTests() {
  console.log('🏗️  Test E2E del Asistente - Residencia San Cristóbal\n');
