...
async function getManagedBuildingCategoriesForTenant(prisma, tenantId) {
...
async function getReferencedExpensesForCategories(prisma, categoryIds) {
...
async function getAllExpenseCategoriesForTenant(prisma, tenantId) {
    return prisma.expenseLedgerCategory.findMany({
        where: {
            tenantId,
            movementType: 'EXPENSE',
...
async function dryRunAllTenants(prisma) {
    ...
    for (const tenant of tenants) {
        ...
        const referencedByExpensesCount = await getReferencedExpensesForCategories(prisma, categoryIds);
        const referencedByIncomesCount = categoryIds.length
            ...
            : 0;
        const recommendedMode = referencedByExpensesCount > 0 ? 'safe' : 'wipe';
        reports.push({
            tenantId: tenant.id,
...
async function dryRunTenant(prisma, tenantId, mode) {
    ...
    const referencedCount = await getReferencedExpensesForCategories(prisma, categories.map((c) => c.id));
    let finalMode;
    if (mode === 'auto') {
...
async function applyWipe(prisma, tenantId) {
    const categories = await getManagedBuildingCategoriesForTenant(prisma, tenantId);
    const categoryIds = categories.map((c) => c.id);
...
async function applySafe(prisma, tenantId) {
...
async function main() {
    ...
    try {
        ...
        if (argv.dryRun) {
            ...
            const report = await dryRunTenant(prisma, argv.tenantId, argv.mode);
...
//# sourceMappingURL=reset_building_expense_categories.js.map