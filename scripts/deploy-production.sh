...
readonly BACKUP_SCRIPT='/opt/pawtech/backups/scripts/backup-postgres.sh'
...
readonly RECORD="$DEPLOYMENTS_DIR/deploy-$(date -u +%Y%m%dT%H%M%SZ)-$TARGET_SHA.txt"
  ...
  while IFS= read -r path; do
    case "$path" in
      .env|.env.*|*/.env|*/.env.*|*.pem|*.key|*.p12|*.pfx|*.crt|*.log|*.dump|*.sql|*.bak|*.backup)
        fail "Sensitive ignored file would enter the Docker context: $path"
        ;;
    esac
...
[[ "$MIGRATION_COUNT" =~ ^[0-9]+$ ]] || fail "Unable to verify migration count"
