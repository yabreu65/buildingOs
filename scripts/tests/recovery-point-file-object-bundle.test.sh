#!/usr/bin/env bash
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; LIB="$ROOT/scripts/lib/recovery-point-file-object-bundle.sh"; PORTABLE="$ROOT/scripts/lib/recovery-point-portable-stat.sh"
...
sha(){ if command -v sha256sum >/dev/null; then sha256sum "$1"|awk '{print $1}'; else shasum -a 256 "$1"|awk '{print $1}'; fi; }; mode(){ recovery_point_portable_stat_mode "$1"; }; owner(){ recovery_point_portable_stat_uid "$1"; }
