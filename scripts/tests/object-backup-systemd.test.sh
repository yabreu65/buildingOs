...
assert_contains 'service uses protected working directory' 'WorkingDirectory=/var/lib/buildingos-object-backup' "$SERVICE"
assert_contains 'service invokes protected object backup primitive' 'ExecStart=/usr/local/libexec/buildingos-backup/backup-object-storage.sh' "$SERVICE"
assert_absent 'service does not require the application checkout script' '/opt/pawtech/apps/buildingos/buildingos-app/scripts/backup-object-storage.sh' "$SERVICE"
